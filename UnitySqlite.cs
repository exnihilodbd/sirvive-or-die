...
public class Unity : MonoBehaviour
{
    ...
    public InputField t_name, t_Address;
    private InputField t_id;
    ...
    private void insert_function(string name, string Address)
    {
        using (dbconn = new SqliteConnection(conn))
        {
            ...
            sqlQuery = string.Format("insert into Login (username, password) values (\"{0}\",\"{1}\")", name, Address);// table name
    ...
    private void reader_function()
    {
        ...
        using (dbconn = new SqliteConnection(conn))
        {
            ...
            string sqlQuery = "SELECT  username, password " + "FROM Login";// table name
            dbcmd.CommandText = sqlQuery;
            ...
            while (reader.Read())
            {
                ...
                Debug.Log(" username =" + Namereaders + "Password=" + Addressreaders);
            }
            reader.Close();
            reader = null;
            dbcmd.Dispose();
